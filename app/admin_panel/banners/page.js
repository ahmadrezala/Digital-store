...
import Bannerdelete from "./delete/[id]/page";
...
const bannerlist = () => {
    ...
    function handlePageClick(data){
        ...
        fetch(`http://127.0.0.1:8000/api/banners?page=${data.selected + 1}`)
        .then(res => res.json())
        .then(data => { 
    ...
    return(
        ...
        <div className="pt-[90px] w-[90%]">
            <h2 className="text-white text-[24px] font-bold capitalize">banners list</h2>
        ...
        <div className="w-[90%] bg-[#141a29] mt-[60px] p-[36px] rounded-[1.5rem]">
            <div className="w-full capitalize flex justify-between text-white border-b-[1px] pb-[20px]">
                <div>
                    <h6 className="text-[#d7d7d8] text-[14px]">total banner</h6>
                    <div className="text-[20px]">{count}</div>
                </div>

                <div>
                    <div className="flex items-center">
                        <Link href="/admin_panel/banners/create" className="rounded-[100px] text-[12px] text-white border p-[12px]">
                        create banner
                        </Link>
                    </div>
                </div>
            ...
            {banners && banners.map((banner) => (
                ...
                <div className="flex text-white capitalize" key={banner.id}>
                    <span className="w-[30%] mb-[5px]">{banner.title}</span>
               
                    <Link href={`/admin_panel/banners/edit/${banner.id}`}><i className="bi bi-gear"></i></Link>
