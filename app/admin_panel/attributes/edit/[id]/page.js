...
const Attributeedit = () => {
    ...
    function handelSubmit(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              fetch(`http://127.0.0.1:8000/api/attributes/${attribute.id}` ,{
    ...
    useEffect(()=>{
        fetch(`http://127.0.0.1:8000/api/attributes/${params.id}`)
        .then(res => res.json())
        .then(data => {
            console.log(data)
    ...
    return(
        ...
        <div className="pt-[90px] w-[90%]">
            <h2 className="text-white text-[24px] font-bold capitalize">update attribute</h2>
        ...
        <div className="w-[70%] bg-[#141a29] mt-[60px] p-[36px] rounded-[1.5rem]">
            ...
            <div className="flex justify-between pt-[36px]">
                <div className="w-[48%]">
                    ...
                    <input  onChange={(e)=>{setName(e.target.value)}} defaultValue={attribute.name} className="h-[48px] mt-[4px] w-full rounded-[30px]" type="text"/>
                </div>
