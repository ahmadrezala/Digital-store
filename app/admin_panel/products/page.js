...
import Productdelete from "./delete/[id]/page";
...
const productslist = () => {
    ...
    return(
        ...
        <div className="pt-[90px] w-[90%]">
            <h2 className="text-white text-[24px] font-bold capitalize">products list</h2>
        ...
        <div className="w-[90%] bg-[#141a29] mt-[60px] p-[36px] rounded-[1.5rem]">
            <div className="w-full capitalize flex justify-between text-white border-b-[1px] pb-[20px]">
                <div>
                    <h6 className="text-[#d7d7d8] text-[14px]">total products</h6>
                    <div className="text-[20px]">{count}</div>
                </div>

                <div>
                    <div className="flex items-center">
                        <Link href="/admin_panel/products/create" className="rounded-[100px] text-[12px] text-white border p-[12px]">
                        create products
                        </Link>
                    </div>
                </div>
            ...
            {products && products.map((product) => (
                ...
                <div className="flex text-white capitalize" key={product.id}>
                    <span className="w-[20%] mb-[5px]">{product.name}</span>
                    <span className="w-[20%] mb-[5px]">{product.brand.name}</span>
                    <span className="w-[20%] mb-[5px] overflow-hidden">{product.category.name}</span>
                    <span className="w-[20%] mb-[5px] overflow-hidden">$ {product.price}</span>
                    <Link href={`/admin_panel/products/edit/${product.id}`}><i className="bi bi-gear"></i></Link>
